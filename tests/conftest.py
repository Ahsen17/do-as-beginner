...
@pytest.fixture
def default_container() -> Iterator[Container]:
    """Install a fresh container as the process default and restore it afterwards."""

    previous = DI.get_default_container()
    fresh = Container()
    DI.set_default_container(fresh)
    yield fresh
    DI.set_default_container(previous)
